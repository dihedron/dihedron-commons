 ...
 * @author Andrea Funto'
 */
public class PrintToStdErr extends AbstractTransformation {
	
	/**
	 * The logger.
	 */
	private static final Logger logger = LoggerFactory.getLogger(PrintToStdErr.class);
	...
	protected Scalar transform(ActivityContext context, Scalar scalar) throws ActivityException {
		try {
			...
			System.err.println((scalar != null && scalar.get() != null) ? scalar.get().toString() : "<null>");
			return scalar;
